...
message_type_cat = pd.CategoricalDtype(
    [
        "migrate_from_group",
        "text",
        "pinned_message",
...
def convert_messages(
    df: pd.DataFrame,
) -> typing.Tuple[typing.List[dict], typing.List[dict], dict]:
    messages_out = []
    users_out = []
    ...
    for message in df.itertuples():
        ...
        if message.type == "message":
            ...
            if getattr(message, "forwarded_from", None):
                try:
                    message_dict["forward_from"] = int(
                        message.from_id[4:]
                    )  # username is used in forwarded_from
                except ValueError:
                    pass
            ...
            if message.action == "edit_group_title":
                ...
                or message.action == "join_group_by_link"
            ):
    ...
    user_map = {
        int(i[4:]): df.loc[df["from_id"] == i, "from"].iloc[0]
...
def fix_dtypes_m(df: pd.DataFrame, tz: str) -> pd.DataFrame:
    ...
    df_out.loc[:, "date"] = pd.to_datetime(df_out["date"], utc=False).dt.tz_localize(
        tz=tz, ambiguous=True
    )
...
def update_user_list(
    users: dict[int, tuple[str, str]], engine: sqlalchemy.engine.Engine, tz: str
):
...
def main(json_path: str, db_url: str, tz: str = "Etc/UTC"):
    ...
    df_u = (
        df_u.reset_index()
        .merge(inverse_user_map, how="inner", left_on="user_id", right_on=0)
    ...
    merged = df_u.merge(
        m_ids, how="outer", left_index=True, right_index=True, suffixes=("", "y")
    )
    merged["user_idy"] = pd.to_numeric(merged["user_idy"], errors="coerce").astype(
        "Int64"
    ...
    df_u.to_sql(
        "user_events", engine, if_exists="replace"
    )  # Contains possible updates to existing values
